...
```{r}
plotly::ggplotly(glnc %>% 
  ggplot(aes(continent, r.squared, color = continent, legend = country)) + 
...
```{r}
plotly::ggplotly(tdy %>% mutate(low = estimate - std.error,
                          high = estimate + std.error) %>% 
...
plotly::ggplotly(ggplot(au, aes(.fitted, .resid, color = year)) +
  geom_point()+
  geom_hline(yintercept = 0) +
  geom_smooth(se = FALSE))

plotly::ggplotly(ggplot(au, aes(sample= .std.resid, color = year))+
  stat_qq()+
  geom_abline())

plotly::ggplotly(ggplot(au, aes(.fitted, sqrt(abs(.std.resid))))+
  geom_point()+
  geom_smooth(se = FALSE))

plotly::ggplotly(ggplot(au, aes(.hat, .std.resid, color = year)) +
